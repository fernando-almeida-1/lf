...
R.: Enquanto [destruct] apenas nos permite fazer uma análise em casos dos
    possiveis valores que uma determinada variável pode assumir. [induction]
    além de nos permitir o mesmo, nos fornece uma hipótese de indução e se
    baseia na ideia de que se eu conseguir provar que uma dada proposição é
    verdadeira para um valor inicial e, supondo que essa proposição é válida
    para um valor qualquer, conseguir provar que ela é também verdadeira para
    o próximo valor, então, repetindo esse processo, essa proposição seria
    válida para todos os valores que a variável em questão pode assumir.
*)
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Theorem: Para quaisquer [n] [m] : nat,
    n + m = m + n.

    Proof: Por indução no [n].
    - BASE: Seja [m] : nat. Precisamos mostrar que [0 + m = m + 0].
      Calculemos:
                      0 + m = m             (definição de [+])
                            = m + 0         (add_0_r, com n:=m)
    
    - PASSO INDUTIVO: Seja [n'] : nat tal que para todo [m] : nat, n' + m = m + n' (H.I.).
      Seja [m'] : nat. Calculemos:
                  S n' + m' = S (n' + m')   (definição de [+])
                            = S (m' + n')   (H.I.)
                            = m' + S n'     (plus_n_Sm, com n:=m' e m:=n')
    Qed.
*)
...
(** **** : 2 stars, standard, optional (eqb_refl_informal)

    Write an informal proof of the following theorem, using the
    informal proof of [add_assoc] as a model.  Don't just
    paraphrase the Coq tactics into English!

    Theorem: [(n =? n) = true] para todo [n] : nat.

    Proof: Por indução no [n].
    - BASE: Já temos que [(0 =? 0) = true] de imediato da definição de [=?].

    - PASSO INDUTIVO: Seja [n] : nat tal que (n =? n) = true (H.I.). Calculemos:
               (S n =? S n) = (n =? n)
                            = true          (H.I.)
    Qed.
*)
...
Proof.
  ...
  { intros n. simpl.
    ...
    reflexivity. }
  { intros n. simpl.
    rewrite mul_comm_helper. rewrite IHm'.
    reflexivity. }
Qed.
(** [] *)
...
Proof.
  ...
  { rewrite mul_comm. simpl.
    ...
    { rewrite <- mul_comm_helper.
      ...
      { rewrite <- add_assoc.
        replace (m + (n+n*p)) with ((n+n*p)+m).
        { rewrite <- mul_comm_helper. reflexivity. }
        { rewrite add_comm. reflexivity. } }
      { rewrite add_comm. reflexivity. } } }
Qed.
...
(* R.:  O problema é que os números binários não possuem unicidade. Isto é,
        não são únicos. Por exemplo, o número natural 2 pode ser representado
        por 10, 010, 0010, 00010 etc.
 *)
